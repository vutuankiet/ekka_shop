...
<nav aria-label="..." style="display: flex; justify-content: center; margin: 25px 0 25px 0;">
        <ul class="pagination">
            <li class="page-item <c:if test="${list.page == 1}">disabled</c:if> ">
                <a class="page-link"
                   href="/ekka/${list.path}?page=${list.page - 1}&perpage=${list.perpage}&key=${list.key}&category=${list.category}&genderCategory=${list.genderCategory}&firstPrice=${list.firstPrice}&lastPrice=${list.lastPrice}"
            ...
            <li class="page-item <c:if test="${list.page == list.numberPage}">disabled</c:if>">
                <a class="page-link"
                   href="/ekka/${list.path}?page=${list.page + 1}&perpage=${list.perpage}&key=${list.key}&category=${list.category}&genderCategory=${list.genderCategory}&firstPrice=${list.firstPrice}&lastPrice=${list.lastPrice}">Sau</a>
            </li>
        </ul>
</nav>