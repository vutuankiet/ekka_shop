    ...
    <ul class="ec-pro-pagination-inner">
        <li>
            <a class="<c:if test="${list.page == 1}">disabled</c:if>"
               href="/ekka/${list.path}?page=${list.page>2? list.page- 1: 1}&perpage=${list.perpage}&key=${list.key}&category=${list.category}&genderCategory=${list.genderCategory}&brand=${list.brand}&firstPrice=${list.firstPrice}&lastPrice=${list.lastPrice}"
        ...
        <li>
            <a class="<c:if test="${list.page == list.numberPage}">disabled</c:if>"
               href="/ekka/${list.path}?page=${list.page < list.numberPage ? list.page + 1 : list.page}&perpage=${list.perpage}&key=${list.key}&category=${list.category}&genderCategory=${list.genderCategory}&brand=${list.brand}&firstPrice=${list.firstPrice}&lastPrice=${list.lastPrice}"><i class="fa-solid fa-forward"></i></a>
        </li>
    </ul>
</div>